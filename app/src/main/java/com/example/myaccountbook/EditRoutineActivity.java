...
//                Toast.makeText(RecordActivity.this, "수입으로변경하기" , Toast.LENGTH_SHORT).show();
                ...
                if(type == accountBook.수입){
                    if(반복명.equals("없음")){
                        버튼색상변경(반복버튼,R.color.gray);
                    }
                    else{
                        버튼색상변경(반복버튼, R.color.blue);
                    }
...
//        Toast.makeText(getBaseContext(), "?자산pk " +Integer.toString(자산pk), Toast.LENGTH_SHORT).show();
        ...
        repeat = true;
        반복명 = getIntent().getStringExtra("반복명");
        반복버튼 = findViewById(R.id.button11);
        반복버튼.setText(반복명);

        반복버튼.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                Intent intent = new Intent(EditRoutineActivity.this, SelectRoutineActivity.class);
                startActivityForResult(intent,SelectRoutlineCode);
...
//        Color.BLUE
        ...
        else if(가계부.type == accountBook.고정지출 || 가계부.type == accountBook.변동지출){
            버튼색상변경(반복버튼, R.color.orange);
        }
...
//        Date date = 가계부.parseISODate(날짜);
        ...
        month = calendar.get(java.util.Calendar.MONTH) + 1; // 월은 0부터 시작하므로 +1 해줍니다.
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog dialog = new DatePickerDialog(v.getContext(), new DatePickerDialog.OnDateSetListener() {
                    ...
                    public void onDateSet(DatePicker view, int m_year, int m_month, int dayOfMonth) {
                        ...
                        if(!Routine.반복가능한날짜선택했는지(selectedDate.getTime(),반복명)){

                            Toast.makeText(view.getContext(), "반복 없음으로 변경합니다.", Toast.LENGTH_SHORT).show();
                            repeat = false;
                            반복명 = "없음";
                            ...
                            if (drawableRight != null) {
                                // Tint 색상 설정
                                int tintColor = ContextCompat.getColor(v.getContext(), R.color.gray); // 원하는 색상 지정
...
//                            반복버튼.setCompoundDrawablesWithIntrinsicBounds(drawables[0], drawables[1], drawableRight, drawables[3]);
        ...
        자산종류.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(EditRoutineActivity.this, AssetActivity.class);
...
//                startActivityIfNeeded(intent,REQUEST_ASSET);
        ...
        사진선택.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //PopupMenu 객체 생성
                PopupMenu popup= new PopupMenu(EditRoutineActivity.this, v); //두 번째 파라미터가 팝업메뉴가 붙을 뷰
...
//                            Toast.makeText(RecordActivity.this, "Option 2 selected", Toast.LENGTH_SHORT).show();
        ...
        EditRoutineActivity.NumberTextWatcher textWatcher = new EditRoutineActivity.NumberTextWatcher(금액);
        ...
        if(가계부.이미지1 != null){
            ...
            view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(EditRoutineActivity.this,ImagePageActivity.class);
...
//                        }
                        ...
                        java.util.Calendar m_calendar = java.util.Calendar.getInstance();
                        m_calendar.set(year,month-1,day);
                        Date date = Routine.getTime(m_calendar.getTime(),name);

                        m_calendar.setTime(date);
                        year = m_calendar.get(java.util.Calendar.YEAR);
                        month = m_calendar.get(java.util.Calendar.MONTH) + 1; // 월은 0부터 시작하므로 +1 해줍니다.
...
//            onBackPressed();
    ...
    void 버튼색상변경(Button 버튼, int 색상){
        버튼.setTextColor(getResources().getColor(색상));
//        버튼.setTextColor(색상);
        // 버튼의 CompoundDrawables 가져오기 (좌측, 상단, 우측, 하단)
        Drawable[] drawables = 버튼.getCompoundDrawables();

        // 원하는 Drawable 위치(예: 우측)의 Tint 설정
        Drawable drawableRight = drawables[1]; // 우측 Drawable의 인덱스는 2
        if (drawableRight != null) {
            // Tint 색상 설정
            int tintColor = ContextCompat.getColor(this, 색상); // 원하는 색상 지정
            drawableRight.setTint(tintColor);
//
//                            drawableRight = DrawableCompat.wrap(drawableRight);
//                            DrawableCompat.setTint(drawableRight, tintColor);
